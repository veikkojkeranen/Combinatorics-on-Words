...
size_t localPeak = 0; // Tracks local maximums for Backtracking Telemetry
...
bool extendWord(std::string& currentWord, int targetLength, const std::string& outputFile) {
    ...
    if (currentWord.length() > localPeak) {
        // We are moving forward, push the peak up
        localPeak = currentWord.length();
    } else if (localPeak >= currentWord.length() + 100) {
        // We have backtracked 100 letters from the recent peak
        std::cout << currentWord.length() << " ";
        std::cout.flush(); // Forces the console to print it immediately
        
        // Reset the peak to the current depth to track the NEXT 100-letter drop
        localPeak = currentWord.length(); 
    }
    ...
    if (currentWord.length() > maxDepthReached) {
        ...
        // Append the new record to the log file immediately WITHOUT the extra blank line
...
int main(int argc, char* argv[]) {
    // Default fallback seed if no file is provided
    std::string seed = "bbcccacbcccaaabaaacaaabbbaaacccabcbbbabb";
    int targetLength = 2000; 

    // Override with command-line arguments if provided
    if (argc >= 2) {
        std::string seedFilePath = argv[1];
        std::ifstream seedFile(seedFilePath);
        
        if (seedFile.is_open()) {
            std::string line;
            if (std::getline(seedFile, line)) {
                // Erase any trailing whitespace/newlines from the file read
                line.erase(line.find_last_not_of(" \n\r\t") + 1);
                if (!line.empty()) {
                    seed = line;
                    std::cout << "Loaded seed from file: " << seedFilePath << "\n";
                }
            }
            seedFile.close();
        } else {
            std::cerr << "Warning: Could not open seed file at '" << seedFilePath 
                      << "'. Using default fallback seed.\n";
        }
    }
    ...
    // Use like this: backtracker_v5.exe [PATH_TO_SEED_FILE.txt] 2500
    ...
    localPeak = seed.length(); // Initialize the telemetry peak tracker
    
    std::cout << "Starting seed length: " << seed.length() << "\n";
    // We only print the first 50 chars of the seed to avoid flooding the console
    std::cout << "Starting seed preview: " << seed.substr(0, std::min<size_t>(50, seed.length())) << "...\n";
